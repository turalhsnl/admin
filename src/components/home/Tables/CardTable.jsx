...
const CardTable = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://localhost:${import.meta.env.VITE_API_PORT}/slides`);
    ...
    const isFormValid = formData.title && formData.description && formData.url;

    const createSlide = (e) => {
        e.preventDefault();

        axios.post(`http://localhost:${import.meta.env.VITE_API_PORT}/slides`, formData)
            .then((res) => {
                handleClose();
                fetchData();
            })
    ...
    const handleDelete = (slideId) => {
        ...
        if (shouldDelete) {
            axios.delete(`http://localhost:${import.meta.env.VITE_API_PORT}/slides/${slideId}`)
                .then((res) => {
                    fetchData();
                })
                .catch((error) => {
    ...
    const handleUpdate = () => {
        axios.put(`http://localhost:${import.meta.env.VITE_API_PORT}/slides/${slideId}`, formData)
            .then((res) => {
                fetchData();
                handleClose();
            })
